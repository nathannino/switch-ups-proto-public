...
func handle_client(battle_log : Dictionary, battle_root : Node) :
	...
	var change = battle_log["energy_change"]
	spirit.current_stamina = battle_log["new_energy"]
	if change > 0 :
		battle_root.enter_log_text("TR_BTLLOG_AC_CH_ENERGY_PLUS", {"amount":abs(change)}, {"spirit":SpiritDictionary.spirits[spirit.key].name}, 1)
	elif change < 0 :
		battle_root.enter_log_text("TR_BTLLOG_AC_CH_ENERGY_MINUS", {"amount":abs(change)}, {"spirit":SpiritDictionary.spirits[spirit.key].name}, 1)
	else :
